<template>
  <div style="text-align: center">기간별 예약수 통계 - 연령, 성별별</div>
  <div class="q-pa-md">
...
const fetchCustomerVisits = async () => {
  ...
  try {
    const response = await axios.get(
      `/api/room/reservation/statistics?startDate=${startDate.value}&endDate=${endDate.value}`
    );
    ...
    const combinedRates = [];
    for (let i = 0; i < femaleRates.length; i++) {
      combinedRates.push(femaleRates[i], maleRates[i]);
    }

    chartData.value.datasets[0].data = femaleRates;
    chartData.value.datasets[1].data = maleRates;

    loaded.value = true; // Ensure this is set after updating chartData
  } catch (error) {
